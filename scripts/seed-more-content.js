...
// already exist (safe to run more than once). Run via Plesk's "Run script"
// button in the Node.js panel, or locally with: node scripts/seed-more-content.js
...
async function run() {
  ...
  function reviewExists(slug) {
    return !!db.prepare('SELECT id FROM reviews WHERE slug = ?').get([slug]);
  }
  function couponExists(slug) {
    return !!db.prepare('SELECT id FROM coupons WHERE slug = ?').get([slug]);
  }
...
<p><strong>Worth knowing:</strong> Westwind frequently bundles this set with a free accessory (a Pathfinder backpack, hydration pack, or tank bag) depending on current promotions, so it's worth checking what's included before checkout.</p>`,
  ...
  coupons.forEach((c) => {
    if (!couponExists(c.slug)) {
      insertCoupon.run(c);
      addedCoupons++;
      console.log('Added coupon:', c.title);
    } else {
